...
class Video(models.Model):
    ...
    preview_url_manual = models.URLField(blank=True, null=True, verbose_name="Manual preview URL", help_text="If set, this URL will be used instead of the automatic preview")
    ...
    def save(self, *args, **kwargs):
        if self.preview_url_manual:
            ...
            if re.search(r'(youtube\.com|youtu\.be)', self.url):
                ...
                self.preview_url = f'https://img.youtube.com/vi/{self.video_id}/hqdefault.jpg' if self.video_id else None
            elif 'drive.google.com' in self.url:
                self.source = 'gdrive'
                match = re.search(r'/d/([^/]+)', self.url)
                self.video_id = match.group(1) if match else None
                self.preview_url = f'https://drive.google.com/thumbnail?id={self.video_id}' if self.video_id else None
            # else:
            #     self.source = 'local'    
    ...
    def embed_url(self):
        ...
        elif self.source == 'gdrive' and self.video_id:
